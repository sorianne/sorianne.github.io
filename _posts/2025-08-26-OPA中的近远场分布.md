...
> 光栅的远场辐射原理类似于Fraunhofer多缝衍射。光的干涉使得辐射到远场的光能量在特定角度汇聚，形成mainlobe。在其他的角度，由于不理想的干涉效应，会产生grating lobe和sidelobe。

<img width="500" height="250" alt="image" src="https://github.com/user-attachments/assets/f6188647-f52f-4dd7-9517-0ed0061bf748" align = "middle"/>
...
如果接收平面与衍射孔径的距离较近，接收面上的pattern称为近场图。pattern的光场复振幅为：

$$
\begin{equation}
  e(x_{1},y_{1})=\frac{1}{j\lambda z_{0}}e^{jkz_{0}}e^{\frac{jk}{2z_{0}}(x_{1}^{2}+y_{1}^{2})} \iint e(x_{0},y_{0})e^{\frac{jk}{2z_{0}}(x_{0}^{2}+y_{0}^{2})}e^{\frac{-jk}{z_{0}}(x_{1}x_{0}+y_{1}y_{0})}dx_{0}dy_{0}
  \label{eq:1}
\end{equation}
$$
...
如果接收面与衍射孔径间的距离足够大时，光束辐射可以简化为平面波的传播，接收面上的pattern称为远场图。pattern的光场复振幅为：

$$
\begin{equation}
  e(x_{1},y_{1})=\frac{1}{j\lambda z_{0}}e^{jkz_{0}}e^{\frac{jk}{2z_{0}}(x_{1}^{2}+y_{1}^{2})} \iint e(x_{0},y_{0})e^{\frac{-jk}{z_{0}}(x_{1}x_{0}+y_{1}y_{0})}dx_{0}dy_{0}
  \label{eq:2}
\end{equation}
$$

> 由于远场的条件删除了 $$e^{\frac{jk}{2z_{0}}(x_{0}^{2}+y_{0}^{2})}$$，约等于1


#### 近场 to 远场 | Fourier Transform

远场辐射光强与近场辐射光强之间存在Fourier变换关系，即远场分布可以通过傅里叶变化处理近场分布得到。根据\eqref{eq:2}可得，具有规则形状的辐射孔径可直接通过公式计算远场光强分布（可积分）。一般情况下，衍射孔径可近似为长度为$l$、宽度为$w$的矩形，$sin\theta _{x} =x_{1}/z_{0}$和$sin\theta _{y} =y_{1}/z_{0}$，其远场衍射区域中任意点的光场复振幅为：

$$
\begin{equation}
  e(x_{1},y_{1})=\frac{1}{j\lambda z_{0}} e^{jkz_{0}} e^{\frac{jk}{2z_{0}} (x_{1}^{2}+y_{1}^{2})} w\cdot l\cdot sinc(\frac{klsin\theta_{x}}{2})\cdot sinc(\frac{kwsin\theta_{y}}{2})
  \label{eq:3}
\end{equation}
$$

当矩形衍射孔径在某一方向上的尺寸远大于另一方向时，假设该方向为$x$，则可以将其等效为单缝衍射模型。在这种情况下，$y$方向上的衍射效应忽略不计，只有$x$方向的衍射对光斑变化产生显著影响，导致衍射光斑主要集中在$x$方向


## 远场光束扫描原理

<img width="550" height="490" alt="image" src="https://github.com/user-attachments/assets/aa1d8223-73fa-41b8-a446-908e9eb4f706" align = "middle" />

1D 1xN OPA 由 N 个array element排列组成，其解析计算模型可简化为多个array element的光场复振幅的叠加

假设每个element的尺寸无限小，第$i (i=1, 2, 3,…, N)$个element辐射的光信号振幅为$A_{i}$
